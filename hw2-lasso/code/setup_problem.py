...
def get_target_and_featurizer(num_basis_fns = 100, num_nonzero = 10, coefs_true=None):
    ...
    all_basis_fns = [step_fn_generator(stepLoc=s)
                     for s in np.linspace(0, 1, num_basis_fns, endpoint=False)] ## s = 0일때는 무조건 1이므로 이부분이 bias를 나타내는 부분이다.
    ...
    def featurize(x):
        ...
        for ftr_num in range(num_basis_fns):
            X_ftrs[:, ftr_num] = all_basis_fns[ftr_num](x) ## 0~1구간을 num_basis_fns개수만큼 균일하게 쪼갠다.
        return X_ftrs
