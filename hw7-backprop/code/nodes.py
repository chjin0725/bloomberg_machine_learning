...
class ValueNode(object):
    ...
    def __init__(self, node_name):
        ...
        self.out = None ## foward에 쓸 값
        self.d_out = None ## backward에 쓸 값
...
class VectorScalarAffineNode(object):
    ...
    def forward(self):
        ...
        self.d_out = np.zeros(self.out.shape) ## backward할 때 업데이트할 수 있도록 foward단계에서 미리 초기화 해 놓는다.
        return self.out
    ...
    def backward(self):
        ...
        d_b = self.d_out
        self.x.d_out += d_x  ## 이전 노드의 d_out에 현재 노드의 gradient 값을 전달한다. 이를 이용해서 체인룰을 계산하는 거임.
        self.w.d_out += d_w  ## 왜 = 으로 안하고 +=으로 했을까? 해당 노드의 gradient가 여러 gradient의 sum으로 표현될 수 있기 때문이다.
        self.b.d_out += d_b  ## 강의자료 13~16, 19~20 참고.

    def get_predecessors(self):
        return [self.x, self.w, self.b] ## 이전 노드들을 리턴.
...
class L2NormPenaltyNode(object):
    ...
    def forward(self):
        self.out = self.l2_reg * self.w.out@self.w.out ## @는 np.dot이랑 같다.
        self.d_out = np.zeros(self.out.shape)
        return self.out
    
    def backward(self):
        d_w = self.d_out * 2 * self.l2_reg * self.w.out ## 앞에서 온 gradient값에 현재 노드의 gradient를 곱한다(체인룰).
        self.w.d_out += d_w
        return self.d_out
...
class AffineNode(object):
    ...
    def backward(self):
        ...
        if len(self.W.out.shape) == 1:
            d_x = self.W.out.T * self.d_out ## W가 array인 경우 
        else:
            d_x = np.dot(self.W.out.T, self.d_out)
        

        
        if d_W.shape[0] == 1:  ## 1 by m 처럼 1차 array형태여야 하는데 2차로 되어 있을경우
            d_W = d_W.flatten()
        self.W.d_out += d_W
        self.x.d_out += d_x
        self.b.d_out += d_b
...
class TanhNode(object):
    ...
    def backward(self):
        d_a = self.d_out * (1 - self.out**2) ## 1-tanh^2
        self.a.d_out += d_a
        return self.d_out
